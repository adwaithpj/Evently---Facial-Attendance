...
class Countdown(ft.UserControl):
    def __init__(self):
        ...
        self.attendance_area_name = ft.Text(
            value=" ", size=30, font_family="DM Sans Bold"
        )
        self.attendance_area_status = ft.Text(
            "Searching!", size=30, font_family="DM Sans Medium"
        )

        # Attendance camera area
        self.img = ft.Image(border_radius=ft.border_radius.all(20))
        self.camera_area = ft.Container(
            col=8, width=1000, content=ft.Column([self.img])
        )
        self.search_icon = ft.Icon(
            name=ft.icons.SEARCH_ROUNDED, color=ft.colors.BLUE, size=130
        )

        self.green_tick = ft.Icon(
            name=ft.icons.CHECK_CIRCLE_ROUNDED, color=ft.colors.GREEN, size=130
        )

        self.yellow_tick = ft.Icon(
            name=ft.icons.CHECK_CIRCLE_ROUNDED, color=ft.colors.YELLOW, size=130
        )

        self.red_cross = ft.Icon(
            name=ft.icons.CANCEL_ROUNDED, color=ft.colors.RED, size=130
        )

        self.attendance_area_icon = ft.Container(content=self.search_icon)
                        ...
                        controls=[
                            ...
                            ft.Container(
                                content=ft.Row(
                                    ...
                                    alignment=ft.MainAxisAlignment.CENTER,
                                    spacing=25,
                                    controls=[self.attendance_area_status],
                                )
                            ),
        ...
        self.camera_attendance_area = ft.Container(
            ...
            content=ft.ResponsiveRow(controls=[self.camera_area, self.attendance_area]),
        )
    ...
    def load_pickle(self):
        ...
        try:
            with open(f"{PICKLED_FILE_PATH}/encoded_data_{event_id}.pkl", "rb") as f:
                ...
                print(
                    "Encoded file loaded"
                )  # Comment out -> after testing/if you don't want to print
        ...
        for encodeFace, faceLoc in zip(self.encodeCurFrame, self.faceCurFrame):
            ...
            if matches[matchIndex]:
                ...
                studentIDs = self.studentIDs[
                    matchIndex
                ]  # check database if the student ID is present
    ...
    def update_timer(self):
        ...
        while running:
            ...
            if len(self.faceCurFrame) == 0:
                ...
                self.encodeCurFrame = face_recognition.face_encodings(
                    img_frame, self.faceCurFrame
                )  # changes
...
class Facerecognitionscreen:
    ...
    def view(self, page: ft.Page, params: Params, basket: Basket):
        ...
        self.report_problem = ft.IconButton(
            icon=ft.icons.REPORT, icon_size=25, url="https://x.com/AdwaithPj"
        )

        self.appbar = ft.AppBar(
            title=self.event_name_obj,
            leading=ft.IconButton(ft.icons.ARROW_BACK, on_click=change_route),
            center_title=True,
            bgcolor=ft.colors.SURFACE_VARIANT,
            toolbar_height=70,
            actions=[self.toggledarklight, self.report_problem],
            leading_width=80,
        )
        ...
        return ft.View(
            ...
            controls=[self.appbar, self.camera_attendance_container],
        )
