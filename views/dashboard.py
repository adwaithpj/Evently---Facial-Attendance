...
class Dashboard:
    def __init__(self):
        ...
        self.latest_event_name = ft.Text(
            value="Refresh the Page", font_family="DM Sans Bold", size=60
        )
    ...
    def view(self, page: ft.Page, params: Params, basket: Basket):
        ...
        def get_token_data(basket: Basket):
            ...
            try:
                ...
                print(user_token)
                self.user_refresh_token = self.data_taken_from_login["backendTokens"][
                    "refreshToken"
                ]
            ...
            try:
                ...
                if len(data[f"{update}"]) > 0:
                    ...
                    if update == "today" or update == "tomorrow":
                        self.latest_event_status.value = (
                            f"{update.capitalize()}'s Event"
                        )
                    ...
                    self.latest_event_date.value, self.latest_event_time.value = (
                        dt_obj.split("T")
                    )
                    self.latest_event_time.value = self.latest_event_time.value.rstrip(
                        "Z"
                    )
                    self.latest_event_description.value = data[f"{update}"][0][
                        "eventDescription"
                    ]
                    ...
                    print(
                        self.latest_event_status.value
                    )  # Comment out this after testing
                    print(
                        self.latest_event_name.value
                    )  # Comment out this after testing
                    print(
                        self.latest_event_time.value
                    )  # Comment out this after testing
                    print(
                        self.latest_event_description.value
                    )  # Comment out this after testing
        ...
        def check_event(e):
            ...
            try:
                ...
                url = (
                    "https://backend.evently.adityachoudhury.com/api/event/createdByMe"
                )
        ...
        # Logout Function
        self.logout_progressring = ft.ProgressRing(
        ...
        self.profile_button = ft.FloatingActionButton(
            icon=ft.icons.ACCOUNT_CIRCLE, on_click=open_sidebar_profile
        )
        self.sidebar_profile_name = ft.Text(
            value=self.user_name,
            font_family="DM Sans Bold",
        )
        self.sidebar_email = ft.Text(
            value=self.user_email, style=ft.TextStyle(color=ft.colors.GREY, size=12)
        )
                        ...
                        ft.Container(
                            ...
                            content=ft.Column(
                                ...
                                controls=[
                                    ...
                                    ft.Container(self.latest_event_attendance_button),
                                ],
                            ),
                        ),
                    ]
                        ...
                        ft.Container(
                            content=ft.Column(
                                controls=[
                                    self.logo,
                                    ft.VerticalDivider(
                                        width=10, thickness=20, color="black"
                                    ),
